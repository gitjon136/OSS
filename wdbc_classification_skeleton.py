...
def load_wdbc_data(filename):
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            wdbc.data.append([float(num) for num in items[2:]])        # TODO #1) Add 30 attributes (as floating-point numbers)
...
if __name__ == '__main__':
    ...
    # Train a model
    model = naive_bayes.GaussianNB()                          # TODO #2) Find a better classifier (SVC accuracy: 0.902)
    ...
    matx=metrics.confusion_matrix(wdbc.target,predict)
    disp=metrics.ConfusionMatrixDisplay(matx,display_labels=['malignant','benign'])
    disp.plot()
